        ...
        <client-only>
          ...
          <div v-else class="rocket">
            <aside class="sidebar" :class="{ 'is-open': sidebarOpen }">
              ...
              <MessageBox v-if="error" type="is-warning" size="is-small">{{ error }}</MessageBox>
              <ul class="server-list">
                <li
                  ...
                  @click="select(server)"
                >
                  <span class="dot" :class="{ 'is-online': server.connected }"/>
...
export default {
  ...
  setup() {
    ...
    useHead({
      title: "Rocket - Meta Construct",
      bodyAttrs: { class: "rocket-page" },
    });
    return useUser();
  },
  data() {
  ...
  methods: {
    async loadServers() {
      try {
        const data = await this.$mc("/console/servers");
        this.servers = data;
    ...
    pollStatus() {
      ...
      const tick = async () => {
        ...
        try {
          const data = await this.$mc(`/console/status/${encodeURIComponent(key)}`);
          ...
          const code = err.response && err.response.status;
          if (code === 401 || code === 403) this.fetchUser();
