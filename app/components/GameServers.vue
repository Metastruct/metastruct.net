  ...
  <div v-if="games.length" class="tile is-parent is-vertical is-4 game-servers">
    ...
    >
      ...
      >
        ...
        <MdiIcon class="game-chevron" icon="chevron-down" size="is-small" />
      </button>
      <transition
        name="game-collapse"
        @enter="onEnter"
...
.game-servers {
  ...
  // within itself, so the whole category sticks, against .game-servers.
  .game-category:not(.is-open) {
    ...
    // sits above, so no sliver of scrolling content between the two, and no
